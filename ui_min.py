...
class BibleReader:
	...
	def load_data(A):
		...
		try:
			A.df=[]
			with open('data.csv','r',encoding=_P)as C:
				D=csv.DictReader(C)
				for B in D:A.df.append(B)
			A.bibleid_df=[]
			with open('bibleid.csv','r',encoding=_P)as C:
				D=csv.DictReader(C)
